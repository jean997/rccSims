...
oracle.coverage <- apply(zsim, MARGIN=2, FUN=function(zz){
  ci <- rcc::par_bs_ci(beta=zz, theta=theta)$ci
  covered <- (ci[,1] <= theta & ci[,2] >= theta)
  return(covered)
})
...
ggsave(h1, file="~/Dropbox/Confidence_Intervals/for_jcgs/img/example_cov.pdf", heigh=4, width=16, units="in", dpi=300)
ggsave(h2, file="~/Dropbox/Confidence_Intervals/for_jcgs/img/example_width.pdf", heigh=4, width=16, units="in", dpi=300)
ggsave(legend$plot, file="~/Dropbox/Confidence_Intervals/for_jcgs/img/example_legend.pdf", height=1, width=8, units="in", dpi=300)
```
